...
type PurchaseMessage {
  PurchaseFinished(
    ...
    result: Result(
      factos_pog.Append,
      factos_pog.Error(DomainError, DecodeError),
    ),
  )
}
...
pub fn run() -> Result(SaleSummary, factos_pog.Error(DomainError, DecodeError)) {
...
) -> Result(Nil, factos_pog.Error(DomainError, DecodeError)) {
  use _ <- result.try(
...
fn purchase(
  connection: pog.Connection,
  attempt: Int,
) -> Result(factos_pog.Append, factos_pog.Error(DomainError, DecodeError)) {
  ...
  case factos.event_type_name(stored.type_) {
    "TicketSold" -> {
      use buyer <- result.try(
        ...
        |> result.replace_error(InvalidPayload("buyer was not utf-8")),
      )
      Ok(factos.Decoded(
